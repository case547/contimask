...
def process_results_by_file(CV, explainer_list, path="./results.csv"):
    ...
    for cv in range(CV):
        for e, explainer in enumerate(explainer_list):
            metrics[0, e, cv] = data[data.Fold == cv][
                data.Explainer == explainer
            ].AUP.values[0]
            metrics[1, e, cv] = data[data.Fold == cv][
                data.Explainer == explainer
            ].AUR.values[0]
            metrics[2, e, cv] = data[data.Fold == cv][
                data.Explainer == explainer
            ].Information.values[0]
            metrics[3, e, cv] = data[data.Fold == cv][
                data.Explainer == explainer
            ].Entropy.values[0]
    for e, explainer in enumerate(explainer_list):
        ...
        im_avg, im_std = (
            np.mean(metrics[2, e, :]) / 10000,
            np.std(metrics[2, e, :]) / 10000,
        )
        sm_avg, sm_std = (
            np.mean(metrics[3, e, :]) / 1000,
            np.std(metrics[3, e, :]) / 1000,
        )
...
def process_results(CV, explainer_list, path="experiments/results/rare_time"):
    ...
    for e, explainer in enumerate(explainer_list):
        ...
        im_avg, im_std = (
            np.mean(metrics[4, e, :]) / 10000,
            np.std(metrics[4, e, :]) / 10000,
        )
...
def print_results(mask_label, true_label):
    ...
    print(
        f"Saliency AUROC: {metrics.roc_auc_score(true_label.flatten(), mask_label.flatten())}"
    )
