...
def run_experimet(
    ...
    with open(
        f"results/rare_time_value/{some_index}/rare_time_value_{some_index}_params.txt",
        "w",
    ) as f:
    ...
    for i in range(N_ex):
        ...
        def f_to_explain(t, X, data_mask, return_mask=False):
            ...
            t_diffs = torch.cat(
                [torch.ones_like(t[:, :1]), torch.diff(t, dim=1)], dim=1
            )
        ...
        for explainer in explainers:
            ...
            torch.save(
                ...
                f"results/rare_time_value/{some_index}/rare_time_value_{some_index}_{explainer}_cv{i}_fitted.pt",
            )
            torch.save(
                mask.hist,
                f"results/rare_time_value/{some_index}/rare_time_value_{some_index}_{explainer}_cv{i}_hist.pt",
            )
            torch.save(
                true_mask,
                f"results/rare_time_value/{some_index}/rare_time_value_{some_index}_{explainer}_cv{i}_true.pt",
            )
...
if __name__ == "__main__":
    ...
    explainers_sub = get_explainers_sub(
        explainers_options, mask=["MFF"], pert=["D"], optim=["E"]
    )
