...
def run_experimet(
    ...
    with open(
        f"results/rare_time_temp/{some_index}/rare_time_temp_{some_index}_params.txt",
        "w",
    ) as f:
        ...
        for explainer in explainers:
            ...
            if explainer.split("-")[0] == "MT":
                ...
                pert_mask = MaskFunctionFourier(
                    hidden_dim=hidden_dim, features=N_feat, L=L, init_bias=1
                ).to(device)
            elif explainer.split("-")[0] == "MFH":
                pert_mask = MaskFunctionHaar(
                    hidden_dim=hidden_dim, features=N_feat, init_bias=0, levels=12
                ).to(device)
            else:
            ...
            torch.save(
                ...
                f"results/rare_time_temp/{some_index}/rare_time_temp_{some_index}_{explainer}_cv{i}_fitted.pt",
            )
            torch.save(
                mask.hist,
                f"results/rare_time_temp/{some_index}/rare_time_temp_{some_index}_{explainer}_cv{i}_hist.pt",
            )
            torch.save(
                true_mask,
                f"results/rare_time_temp/{some_index}/rare_time_temp_{some_index}_{explainer}_cv{i}_true.pt",
            )
