...
def area_loss_func(
    pert_mask: Callable, time_lower_bound, time_upper_bound, res, device
) -> torch.Tensor:

    ts = torch.linspace(
        time_lower_bound,
        time_upper_bound,
        math.ceil((time_upper_bound - time_lower_bound) / res),
        device=device,
    )
...
def absolute_derivate_loss_func(
    pert_mask: Callable, time_lower_bound, time_upper_bound, res, device
) -> torch.Tensor:
...
class ContiMask(nn.Module):
    ...
    def get_mask(self, t):
        ...
        """
        if isinstance(self.pert_mask, MaskFunction) and isinstance(
            self.perturbation_func, Perturbation_continuous
        ):
            ...
            raise TypeError(
                f"pert_mask must be a nn.Module or a torch.Tensor, got {type(self.pert_mask)}"
            )
    ...
    def attribute(
        ...
        with torch.no_grad():
            self.y_orig = self.forward_func(t=t, X=X, data_mask=data_mask).to(
                self.device
            )
        ...
        def iteration(module):
            ...
            t_pert, X_pert, data_mask_pert = self.perturbation_func.apply(
                self.t, self.X, self.data_mask, module, K=K
            )
            ...
            if isinstance(module, MaskFunction) and isinstance(
                self.perturbation_func, Perturbation_continuous
            ):
                ...
                tv_loss = torch.mean(
                    torch.abs(mask_at_t[..., 1:, :] - mask_at_t[..., :-1, :]),
                    dim=(-1, -2),
                )
                ...
                if target_area is not None:
                    sparsity = torch.abs(sparsity - target_area)
                tv_loss = torch.mean(
                    torch.abs(module(self.t)[..., 1:, :] - module(self.t)[..., :-1, :]),
                    dim=(-1, -2),
                )
                sharpness = -torch.abs(module(self.t) - 0.5).mean(dim=(-1, -2))

            loss = (
                self.deletion_mode * rec_loss
                + 10 * lambda_l1 * sparsity
                + lambda_tv * tv_loss
            ).mean()
            ...
            self.hist = torch.cat(
                (
                    ...
                    torch.tensor(
                        [
                            [self.deletion_mode * rec_loss.mean().detach().cpu()],
                            [lambda_l1 * sparsity.mean().detach().cpu()],
                            [lambda_tv * tv_loss.mean().detach().cpu()],
                            [lambda_sharp * sharpness.mean().detach().cpu()],
                        ]
                    ),
                ),
                dim=1,
        ...
        if optimization_strategy == "gradient":
            ...
            problem = NEProblem("min", self.pert_mask, iteration, device=self.device)
            ...
            def iteration(module):
                ...
                if isinstance(module, MaskFunction) and isinstance(
                    ...
                    tv_loss = torch.mean(
                        torch.abs(
                            module(self.t)[..., 1:, :] - module(self.t)[..., :-1, :]
                        ),
                        dim=(-1, -2),
                    )
                    sharpness = -torch.abs(module(self.t) - 0.5).mean(dim=(-1, -2))
