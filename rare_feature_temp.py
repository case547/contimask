...
def run_experimet(
    ...
    with open(
        f"results/rare_feature_temp/{some_index}/rare_feature_temp_{some_index}_params.txt",
        "w",
    ) as f:
        ...
        for explainer in explainers:
            ...
            if explainer.split("-")[0] == "MT":
                ...
                pert_mask = MaskFunctionMLP(
                    hidden_dim=hidden_dim, features=N_feat, init_bias=1
                ).to(device)
            elif explainer.split("-")[0] == "MFS":
                pert_mask = MaskFunctionSine(
                    hidden_dim=hidden_dim, features=N_feat, init_bias=0
                ).to(device)
            elif explainer.split("-")[0] == "MFF":
                pert_mask = MaskFunctionFourier(
                    hidden_dim=hidden_dim, features=N_feat, init_bias=1, L=24
                ).to(device)
            elif explainer.split("-")[0] == "MFH":
                pert_mask = MaskFunctionHaar(
                    hidden_dim=hidden_dim, features=N_feat, init_bias=0, levels=48
                ).to(device)
            else:
            ...
            torch.save(
                ...
                f"results/rare_feature_temp/{some_index}/rare_feature_temp_{some_index}_{explainer}_cv{i}_fitted.pt",
            )
            torch.save(
                mask.hist,
                f"results/rare_feature_temp/{some_index}/rare_feature_temp_{some_index}_{explainer}_cv{i}_hist.pt",
            )
            torch.save(
                true_mask,
                f"results/rare_feature_temp/{some_index}/rare_feature_temp_{some_index}_{explainer}_cv{i}_true.pt",
            )
...
if __name__ == "__main__":
    ...
    explainers_sub = get_explainers_sub(
        explainers_options, mask=["MFF"], pert=["GB", "FMA", "D"], optim=["E"]
    )
